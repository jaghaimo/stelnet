...
public class TabViewFactory {
    ...
    private void addBuyTab(
        ...
        CommodityTabButton tabButton = getTabButton(
            CommodityTab.BUY,
            Keyboard.KEY_B
        );
    ...
    private Table getBuyTable(float width, MarketRepository marketRepository) {
        TableContent tableContent = new BuyTableContent(
            commodityId,
            marketRepository.getBuyMarkets()
        );
    ...
    private Table getSellTable(float width, MarketRepository marketRepository) {
        TableContent tableContent = new SellTableContent(
            commodityId,
            marketRepository.getSellMarkets()
        );
        return new Table(commodityId, width, 0, tableContent);
    }

    private Table getProfitTable(
        float width,
        MarketRepository marketRepository
    ) {
        TableContent tableContent = new ProfitTableContent(
            commodityId,
            marketRepository
        );
        return new Table(commodityId, width, 0, tableContent);
    ...
    private CommodityTabButton getTabButton(
        CommodityTab currentTab,
